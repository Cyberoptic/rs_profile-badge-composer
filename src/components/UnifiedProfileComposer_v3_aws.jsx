...
const API_CONFIG = {
  ...
  enabled: true // APIを使用する場合はtrue、localStorageのみの場合はfalse
};
...
const UnifiedProfileComposer = () => {
  ...
  // State for saved assets (localStorage + AWS)
    ...
    const initializeAssets = async () => {
      ...
      if (API_CONFIG.enabled) {
        try {
          ...
          const localFrames = JSON.parse(localStorage.getItem('savedFrames') || '[]');
          const localBadges = JSON.parse(localStorage.getItem('savedBadges') || '[]');
          
          setSavedFrames([...localFrames, ...(framesData.assets || [])]);
          setSavedBadges([...localBadges, ...(badgesData.assets || [])]);
          
        } catch (error) {
          console.warn('⚠️ AWS API offline, using localStorage only');
  ...
  const handleSaveFrame = async () => {
    ...
    // localStorageに保存
    const localFrames = JSON.parse(localStorage.getItem('savedFrames') || '[]');
    const updatedLocal = [...localFrames, newFrame];
    localStorage.setItem('savedFrames', JSON.stringify(updatedLocal));
    ...
    if (apiStatus === 'online') {
      try {
        ...
        console.log('✅ Frame saved to AWS');
      } catch (error) {
        console.warn('⚠️ Failed to save to AWS, saved to localStorage only');
      }
    }
    
    setSavedFrames(updatedLocal);
    setSaveFrameName('');
    setShowFrameSaveDialog(false);
    alert('フレームを保存しました！');
  };

  // Save badge (localStorage + AWS)
  const handleSaveBadge = async () => {
    ...
    // localStorageに保存
    const localBadges = JSON.parse(localStorage.getItem('savedBadges') || '[]');
    const updatedLocal = [...localBadges, newBadge];
    localStorage.setItem('savedBadges', JSON.stringify(updatedLocal));
    
    // AWS APIに保存を試みる
    if (apiStatus === 'online') {
      try {
        await apiRequest('/badges', {
          method: 'POST',
          body: JSON.stringify({
            name: saveBadgeName,
            imageData: imageData
        ...
        console.log('✅ Badge saved to AWS');
      } catch (error) {
        console.warn('⚠️ Failed to save to AWS, saved to localStorage only');
      }
    }
    
    setSavedBadges(updatedLocal);
    setSaveBadgeName('');
  ...
  const handleLoadFrame = (frame) => {
    ...
    img.src = frame.imageData || frame.url; // AWS URLまたはlocalStorage data URL
  };
  ...
  const handleLoadBadge = (badge) => {
    ...
    img.crossOrigin = 'anonymous';
    img.src = badge.imageData || badge.url;
  };
    ...
    <div className="w-full">
      ...
      {API_CONFIG.enabled && (
        <div className={`text-xs px-3 py-1 rounded-full inline-block mb-4 ${
          ...
          {apiStatus === 'online' && '☁️ クラウド同期ON'}
          {apiStatus === 'offline' && '💾 ローカル保存モード'}
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto">
                  {savedFrames.map((frame) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() => handleDeleteFrame(frame.id || frame.assetId)}
              ...
              <div>
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto">
                  {savedBadges.map((badge) => (
                    <div
                      key={badge.id || badge.assetId}
                      className="flex items-center gap-2 p-2 bg-zinc-50 rounded-lg hover:bg-zinc-100"
                    >
                      <img
                        src={badge.imageData || badge.url}
                        alt={badge.name}
                        className="w-10 h-10 object-cover rounded"
                      />
                      ...
                      <button
                        onClick={() => handleDeleteBadge(badge.id || badge.assetId)}
                        className="px-2 py-1 text-xs text-red-600 hover:bg-red-50 rounded"
                      >
                        削除
                      </button>
          ...
          <div className="bg-white rounded-lg border border-zinc-200 p-6">
            ...
            <div className="space-y-4">
              ...
              <button
                ...
                className="w-full px-6 py-3 bg-zinc-900 text-white rounded-lg font-semibold hover:bg-zinc-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                {isProcessing ? '処理中...' : `${outputSize}px画像を書き出し`}
              </button>
            </div>
          </div>
        </div>
